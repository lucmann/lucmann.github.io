...
**动态链接器 `/lib64/ld-linux-x86-64.so.2`** 是程序运行时调用的，它由操作系统内核通过 execve 族系统调用加载到内存，进而由它加载其它用到的动态链接库。它本身是一个静态链接库，不依赖其它库。

**静态链接器 `/usr/bin/ld`** 是构建程序或库时调用的，它由编译器(如 gcc)调用, 它本身是一个可执行程序，而且可以读入链接脚本(linker script)来改变它的行为。例如，常见的 `/lib/x86_64-linux-gnu/libc.so` 就是一个**链接器脚本文件(纯ASCII text)**, 而不是~~共享对象(shared object) Binary 文件~~, 这个链接脚本的内容如下
...
虽然静态链接器和动态链接器运行在两个不同的阶段，但静态链接器也可以影响到动态链接器的行为，比如 `-Wl,-rpath,/path/to/lib`, 也就是常说的 Runtime PATH, 这个参数可以让动态链接器去指定**目录**下搜索的目标动态库。
